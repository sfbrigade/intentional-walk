...
function updateContest() {
  return Api.contest.current().then(response => {
...
function createUser(id, name, email, zip, age, gender, gender_other, race, race_other, is_latino) {
  let user = null;
  return write(realm => {
    user = realm.create(
      'AppUser',
...
function startWalk() {
  return getCurrentWalk().then(walk => {
    if (!walk) {
      return Notifications.checkPermissions()
        .then(permissions => {
...
function addCurrentWalkListener(callback) {
  const listener = (results, changes) =>
...
function getWalks() {
  return open().then(realm =>
    realm.objects('IntentionalWalk').sorted([['end', true]]),
  );
}
...
function syncWalks() {
  return getUser().then(user => {
    if (user) {
      return Api.intentionalWalk.get(user.id).then(response => {
